...
pull_clean <- function(df, dswe =1, minPix = 5, maxClouds = 50,
                       maxRef = 10000, minRef = 0) {
  
  require(anytime)
  
  data <- df %>%
    dplyr::filter_at(vars(Blue, Red, Green, Nir), all_vars(!is.na(.))) %>%
    rename(blue=Blue, green=Green, red=Red, nir=Nir, swir1=Swir1, swir2=Swir2, aerosol=Aerosol,
           hillshadow=hillShadow, LS_ID = 'system:index') %>%
    dplyr::filter( 
      pCount_dswe1 >= minPix,
      ...
      TIR1 <= maxRef,
      TIR2 <= maxRef) %>%
    mutate(LS_ID = ifelse(str_extract(LS_ID, "\\d")== "2", paste0("1_", LS_ID, sep=""), LS_ID)) %>%
    separate(LS_ID, into = c("tier", "collection","sat", "pathrow","date", "junk"), sep="_", remove=F) %>%
    mutate(path = as.numeric(substr(pathrow, 1,3))) %>%
    mutate(row = as.numeric(substr(pathrow, 4,6))) %>%
    mutate(date=ymd(date)) %>%
    mutate(time = anytime(time/1000, tz="GMT") ) %>%
    dplyr::select(-tier, -collection, -junk) %>%
...
# Function to clean and filter surface reflectance pull
pull_clean_match <- function(df, dswe =1, minPix = 5, maxClouds = 50,
                       maxRef = 10000, minRef = 0) {
...
sr_57 <- data %>%
  ...
  filter(n_years > 10) %>%
  select(reach_ID, date, sat, count, n_years, blue, red, green, nir, swir1, swir2, TIR1) %>%
...
png("D:/Dropbox/projects/tss_amazon/figs/poly_5_correction.png",
...
#
write_csv(poly_57, "D:/Dropbox/projects/tss_amazon/out/ls_57_quantile_correction.csv")
...
png("D:/Dropbox/projects/tss_amazon/figs/poly_8_correction.png",
    units="in", width = 6, height=4, res = 300)
...
#
write_csv(poly_78, "D:/Dropbox/projects/tss_amazon/out/ls_78_quantile_correction.csv")
...
#
coef_7 <- tibble(band = c("blue", "red", "green", "nir", "swir1", "swir2", "TIR1"), intercept = 0, coef1=1, coef2=0, sat= "LE07")
...
wlnm2RGB <- function(wavelength, Gamma = 0.8, IntensityMax = 1) {
